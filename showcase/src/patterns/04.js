import React, {
  ...
  const MediumClap = ({ children, onClap, style : userStyles = {} }) => {
    ...
    const [{ clapRef, clapTotalRef, clapCountRef }, setRefState] = useState({});
    ...
    return (
      <Provider value={memoizedValue}>
        <button ref={setRef} data-refkey='clapRef' className={styles.clap} onClick={handleClapClick} style={userStyles}>
          {children}
        </button>
      </Provider>
    );
    ...
    return (
      <div style={{width: '100%'}}>
        <MediumClap onClap={handleClap} style={{border: "1px solid red"}}>
          ...
          <MediumClap.CountTotal style={{background: '#8cacea', color: 'black'}} />
        </MediumClap>
